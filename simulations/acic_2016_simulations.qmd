...
for(i in dgp){
  ...
  for(j in 1:100){
    ...
    runtimes["Krls:single","time"] <- runtime[2]
    ...
    runtime <- proc.time() - start_time
    runtimes["Krls:cross","time"] <- runtime[2]
    ...
    runtime <- proc.time() - start_time
    runtimes["SL:single","time"] <- runtime[2]
    ...
    runtime <- proc.time() - start_time
    runtimes["SL:cross","time"] <- runtime[2]
    ...
    bal_ydhat_sl <- weightit(formula = z ~ y1hat_sl + y0hat_sl,
                             ...
                             method = "ebal", data = data)
    runtime <- proc.time() - start_time
    runtimes["Bal:SL:Yd","time"] <- runtime[2]
    ...
    runtime <- proc.time() - start_time
    runtimes["Krls:ps","time"] <- runtime[2]
    # Predicted probabilities
    ...
    ipw_sl <- weightit(formula = cobalt::f.build("z", names(covs)),
                       ...
                       data = data)
    runtime <- proc.time() - start_time
    runtimes["Bal:SL:ps","time"] <- runtime[2]
    ...
    runtime <- proc.time() - start_time
    runtimes["Krls:pps","time"] <- runtime[2]
    # Predicted probabilities
    ...
    runtime <- proc.time() - start_time
    runtimes["SL:pps","time"] <- runtime[2]
    ...
    ippw_sl <- weightit(formula = cobalt::f.build("z", names(covs)),
                        ...
                        data = data)
    runtime <- proc.time() - start_time
    runtimes["Bal:SL:pps","time"] <- runtime[2]
...
library(ggplot2)
# Add DML results
results_dml <- readRDS("/Users/pablogeraldo/Downloads/results_dml")

for(j in 1:100){
  results[Estimator == 'DR:DML:LogLin' & 
            DGP == 1 & Simulation == j ,'Bias'] <-
    results_dml[[j]][[2]]-ate
  
  results[Estimator == 'DR:DML:xgboost' & 
            DGP == 1 & Simulation == j ,'Bias'] <-
    results_dml[[j]][[6]]-ate
  
  results[Estimator == 'DR:DML:SL' & 
            DGP == 1 & Simulation == j ,'Bias'] <-
    results_dml[[j]][[1]]-ate
}
...
# results_clean |> 
#   dplyr::filter(!grepl("DR:DML", Estimator), Estimator!="DR:TMLE:grf") |> 
#   dplyr::mutate(Bias_reduction = Bias*100/results_clean[Estimator=="Diff-in-means", "Bias"]) |> 
#   ggplot(aes(y=Estimator, x=Bias_reduction, color = type)) +
#   geom_boxplot() +
#   theme_bw() +
#   facet_wrap(~DGP)
...
results_clean |> 
  dplyr::filter(!grepl("DR:DML", Estimator), Estimator!="DR:TMLE:grf") |> 
  dplyr::group_by(Estimator, DGP) |> 
  dplyr::reframe(RMSE = sqrt(mean(Bias^2)), type = type) |> 
  dplyr::arrange(RMSE) |> #print()
