...
for(i in dgp){
  ...
  for(j in 1:100){
    ...
    linlog_pps <- lrn_glm$train(task = task_ps)
    ...
    lasso_pps <- lrn_lasso$train(task = task_ps)
    ...
    xgb_pps <- lrn_xgb$train(task = task_ps)
    ...
    earth_pps <- lrn_earth$train(task = task_ps)
    data$pps_earth <- earth_pps$predict()
    
    # IPPW:SL
    # It has access too all the y1 and y0 hats from the models in SL
