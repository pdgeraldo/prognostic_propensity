...
estimators <- c("Diff-in-means","Coeff:Linear","Coeff:Linear:Lin","Coeff:Lasso",
                ...
                "Bal:Y0:earth","Bal:Y0:Krls","Bal:Y0:SL",
...
for(i in dgp){
  ...
  for(j in 1:5){
    ...
    #esl <- Lrnr_sl$new(stack, metalearner = cv_selector)
    ...
    sl_y <- sl_nnls$train(task = task_y)
    #sl_y <- stack_meta$train(task = task_y)
    ...
    # Lasso, g-comp
    y1hat_lasso <- sl_y$learner_fits$lasso$predict(task = task_y1hat)
    y0hat_lasso <- sl_y$learner_fits$lasso$predict(task = task_y0hat)
    results[Estimator == 'G-comp:single:Lasso' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      mean(y1hat_lasso - y0hat_lasso)-ate
    
    # xgboost
    y1hat_xgb <- sl_y$learner_fits$xgboost$predict(task = task_y1hat)
    y0hat_xgb <- sl_y$learner_fits$xgboost$predict(task = task_y0hat)
    results[Estimator == 'G-comp:single:xgboost' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      mean(y1hat_xgb - y0hat_xgb)-ate
    
    # Earth
    y1hat_earth <- sl_y$learner_fits$earth$predict(task = task_y1hat)
    y0hat_earth <- sl_y$learner_fits$earth$predict(task = task_y0hat)
    results[Estimator == 'G-comp:single:earth' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      mean(y1hat_earth - y0hat_earth)-ate
    ...
    # Lasso, g-comp
    y1xhat_lasso <- sl_y1$learner_fits$lasso$predict(task = task_yxhat)
    y0xhat_lasso <- sl_y0$learner_fits$lasso$predict(task = task_yxhat)
    results[Estimator == 'G-comp:cross:Lasso' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      mean(y1xhat_lasso - y0xhat_lasso)-ate
    
    # xgboost
    y1xhat_xgb <- sl_y1$learner_fits$xgboost$predict(task = task_yxhat)
    y0xhat_xgb <- sl_y0$learner_fits$xgboost$predict(task = task_yxhat)
    results[Estimator == 'G-comp:cross:xgboost' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      mean(y1xhat_xgb - y0xhat_xgb)-ate
    
    # Earth
    y1xhat_earth <- sl_y1$learner_fits$earth$predict(task = task_yxhat)
    y0xhat_earth <- sl_y0$learner_fits$earth$predict(task = task_yxhat)
    results[Estimator == 'G-comp:cross:earth' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      mean(y1xhat_earth - y0xhat_earth)-ate
    ...
    results[Estimator == 'G-comp:cross:SL' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      mean(y1xhat_sl - y0xhat_sl)-ate
    
    # Full sample, model-based for all
    # mean(y1xhat_sl)-mean(y0xhat_sl)-ate
    # Prediction for the missing PO only 
    # (Alternative, not used, not good not terribly bad)
    #y1xhat_sl_imp <- ifelse(data$z==1, data$y, y1xhat_sl)
    #y0xhat_sl_imp <- ifelse(data$z==0, data$y, y0xhat_sl)
    #mean(y1hat_sl_imp - y0hat_sl_imp)-ate
    # Predictions only in the complement (very bad!)
    #mean(y1xhat_sl[data$z==0])-mean(y0xhat_sl[data$z==1])-ate
    ...
    ps_lpm <- lm(cobalt::f.build("z", names(covs)), data = data) |> 
    ...
    ps_krls_lpm <- predict.gam(fit_gKRLS_lpm, type = "response")
    # Fit the model (appropriate link function)
    fit_gKRLS_binom <- mgcv::gam(fmlps, family = "binomial", data = data)
    # Predicted probabilities
    ps_krls_binom <- predict.gam(fit_gKRLS_binom, type = "response")
    ...
    sl <- Lrnr_sl$new(stack_meta, metalearner = cv_selector)
    
    # Now train the new task
    # First: single model for the outcome
    start_time <- proc.time()
    sl_ps <- sl$train(task = task_ps)
    runtime <- proc.time() - start_time
    runtime
