...
metrics <- c("DGP","Simulation","ATE","ATT","ATC",
             "Estimator","Bias","RMSE","MAE","RESS","Balance","Time")
...
for(i in dgp){
  ...
  for(j in 1:100){
    ...
    y1hat_lin <- predict(coeff_lin, type = response, newdata = dataZ1)
    y0hat_lin <- predict(coeff_lin, type = response, newdata = dataZ0)
    ...
    y1hat_krls <- predict(fit_gKRLS, type = "response", newdata = dataZ1)
    y0hat_krls <- predict(fit_gKRLS, type = "response", newdata = dataZ0)
    ...
    ipw_logit <- weightit(formula = cobalt::f.build("z", names(covs)),
                          ps = sl_ps$learner_fits$glm$predict(),
                          stabilize = TRUE,
                          data = data)
    ...
    ipw_lasso <- weightit(formula = cobalt::f.build("z", names(covs)),
                          ps = sl_ps$learner_fits$lasso$predict(),
                          stabilize = TRUE,
                          data = data)
    ...
    ipw_xgb <- weightit(formula = cobalt::f.build("z", names(covs)),
                        ps = sl_ps$learner_fits$xgboost$predict(),
                        stabilize = TRUE,
                        data = data)
    ...
    ipw_earth <- weightit(formula = cobalt::f.build("z", names(covs)),
                          ps = sl_ps$learner_fits$earth$predict(),
                          stabilize = TRUE,
                          data = data)
