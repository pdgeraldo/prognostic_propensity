...
```{r}
# Install packages required for TMLE
...
library(ddml) # Alternative for double ML

# For parallel processing (not working)
#library(future)
#ncores <- availableCores()-1
#plan(multicore, workers = ncores)
# Set necessary max memory for kernels
#options(future.globals.maxSize= 2500*1024^2)

# Define path for data
path <- "/Users/pdgeraldo/Documents/data_cf_all/"
...
# Define number of simulations and number of DGPs to be tested
n_sims <- 10#100
n_dgps <- 20#20

# Define estimators and metrics to be used
estimators <- c("Diff-in-means",
                "Coeff:Linear","Coeff:Linear:Lin",
                "G-comp:single:xgboost","G-comp:single:earth",
                "G-comp:single:Krls","G-comp:single:bart",
                "Bal:Ebal:raw", "Bal:Ebal:expand","Bal:Yd:Lin",
                "Bal:Yd:xgboost","Bal:Yd:earth",
                "Bal:Yd:Krls","Bal:Yd:bart","Bal:Yd:all",
                "Bal:IPW:raw", "Bal:IPW:expand",
                "Bal:IPW:Lin","Bal:IPW:Lasso","Bal:IPW:xgboost",
                "Bal:IPW:earth","Bal:IPW:Krls","Bal:IPW:bart","Bal:IPW:all",
                "DR:AIPW:LogLin",
                "DR:AIPW:xgboost","DR:AIPW:bart",
                "DR:AIPW:earth","DR:AIPW:Krls","DR:AIPW:SL",
                "DR:TMLE:LogLin","DR:TMLE:xgboost","DR:TMLE:bart",
...
for(i in dgp){
  ...
  for(j in 1:n_sims){
    ...
    # bart
    data$y1hat_bart <- sl_y$learner_fits$bart$predict(task = task_y1hat)
    data$y0hat_bart <- sl_y$learner_fits$bart$predict(task = task_y0hat)
    results[Estimator == 'G-comp:single:bart' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      mean(data$y1hat_bart - data$y0hat_bart)-ate
    ...
    # Yd balance: bart
    bal_ydhat_bart <- weightit(formula = z ~ y1hat_bart + y0hat_bart,
                              #moments = 3, int = TRUE,
                              method = "ebal", data = data)
    results[Estimator == 'Bal:Yd:bart' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      lm(y ~ z, weights = get.w(bal_ydhat_bart), data = data)$coefficients["z"]-ate
    ...
    wlogit <- get_w_from_ps(data$ps_logit, treat = data$z,
                    estimand = "ATT")
    ...
    results[Estimator == 'Bal:IPW:Logit' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      lm(y ~ z, weights = wlogit, data = data)$coefficients["z"]-ate
    ...
    wxgb <- get_w_from_ps(data$ps_xgb, treat = data$z,
                    estimand = "ATT")
    ...
    results[Estimator == 'Bal:IPW:xgboost' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      lm(y ~ z, weights = wxgb, data = data)$coefficients["z"]-ate
    
    # IPW: bart
    data$ps_bart <- sl_ps$learner_fits$bart$predict()
    wbart <- get_w_from_ps(data$ps_bart, treat = data$z,
                    estimand = "ATT")
    ipw_bart <- weightit(formula = cobalt::f.build("z", names(covs)),
                        ps = "ps_bart",
                        stabilize = TRUE,
                        data = data)
    results[Estimator == 'Bal:IPW:bart' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      lm(y ~ z, weights = wbart, data = data)$coefficients["z"]-ate
    
    
    # IPW: earth
    data$ps_earth <- sl_ps$learner_fits$earth$predict()
    wearth <- get_w_from_ps(data$ps_earth, treat = data$z,
                    estimand = "ATT")
    ...
    results[Estimator == 'Bal:IPW:earth' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      lm(y ~ z, weights = wearth, data = data)$coefficients["z"]-ate
    
    # IPW: krls
    wkrls <- get_w_from_ps(data$ps_krls_binom, treat = data$z,
                    estimand = "ATT")
    ...
    results[Estimator == 'Bal:IPW:Krls' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      lm(y ~ z, weights = wkrls, data = data)$coefficients["z"]-ate
    
    # IPW: SL 
    data$ps_sl <- sl_ps$predict()
    #start_time <- proc.time()
    wsl <- get_w_from_ps(data$ps_sl, treat = data$z,
                    estimand = "ATT")
    ...
    results[Estimator == 'Bal:IPW:SL' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      lm(y ~ z, weights = wsl, data = data)$coefficients["z"]-ate
    ...
    # "DR:AIPW:bart"
    results[Estimator == 'DR:AIPW:bart' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      mean((data$z*(data$y-data$y1hat_bart)/data$ps_bart+data$y1hat_bart)-
             ((1-data$z)*((data$y-data$y0hat_bart)/(1-data$ps_bart))+data$y0hat_bart))-ate
    ...
    task_pps <- sl3_Task$new(
      ...
      outcome = "z",
      outcome_type = "binomial",
      covariates = c("y1hat_bart","y0hat_bart"
      ))
    bart_pps <- lrn_bart$train(task = task_pps)
    data$pps_bart <- bart_pps$predict()
   ...
   # runtimes[DGP == i & Simulation == j & algorithm == "SL:pps","time"] <- runtime[2]
    ...
    # IPPW: bart pscore
    ippw_bart <- weightit(formula = cobalt::f.build("z", names(covs)),
                           ps = "pps_bart",
                           stabilize = TRUE,
                           data = data)
    results[Estimator == 'IPPW:bart' & 
              DGP == i & Simulation == j ,'Bias'] <- 
      lm(y ~ z, weights = get.w(ippw_bart), data = data)$coefficients["z"]-ate
...
ggplot(results[!is.na(results$Bias) & results$Estimator!="Bal:Ebal:expand" & abs(results$Bias)<1000,], aes(y = Estimator, x = Bias)) + geom_boxplot() + geom_vline(xintercept=0, color="red") + theme_bw()

results_filter <- results |>  dplyr::filter(!is.na(Bias))

results_filter |> ggplot(aes(x = Bias, y = Estimator)) +
